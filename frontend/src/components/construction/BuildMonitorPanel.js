...
import {
  CalendarPlus, ClipboardCheck, Gauge, HardHat, Lock, RefreshCw, ShieldAlert, TimerOff,
} from "lucide-react";
    ...
    <div data-testid={BUILD.monitorPanel} className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border bg-card p-2.5">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {can.configure ? (
            <Button size="sm" data-testid={BUILD.generateBtn} onClick={() => setGenOpen(true)}>
              <CalendarPlus className="mr-1 h-3.5 w-3.5" /> Buat jadwal unit
            </Button>
          ) : null}
        </div>
      </div>
